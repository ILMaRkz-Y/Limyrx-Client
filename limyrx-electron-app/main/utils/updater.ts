...
async function downloadAsarUpdate(
  ...
  const github = `https://github.com/ILMaRkz-Y/Limyrx-Client/releases/download/v${version}/${file}`
...
async function hintUserDownload(): Promise<void> {
  shell.openExternal('https://client.limyrx.online')
}
...
async function downloadAppInstaller(
  ...
  const destination = join(app.getPath('downloads'), 'Limyrx Client.appinstaller')
      ...
      Object.assign(executor, {
        ...
        createRequest: (options: any, callback: any) => {
          if (gfw.inside) {
            ...
            options.pathname = `/Soft_Mirrors/github-release/ILMaRkz-Y/Limyrx-Client/LatestRelease/${basename(options.pathname)}`
