...
function createController(): WindowController {
  ...
  return {
    ...
    setUiZoom(zoomPercent: number) {
      return ipcRenderer.invoke('set-ui-zoom', zoomPercent)
    },
    agentRequest(payload) {
      return ipcRenderer.invoke('agent-request', payload)
    },
    agentAbort(requestId: string) {
