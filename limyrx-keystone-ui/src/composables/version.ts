...
import { ForgeVersion, LimyrxClientServiceKey, VersionMetadataService, VersionMetadataServiceEventMap, VersionMetadataServiceKey } from '@xmcl/runtime-api'
...
/**
 * The Limyrx Client platform versions. The manifest is fetched from the
 * LimyrxClientService (hosted on GitHub / jsDelivr); `data` holds the whole
 * manifest so consumers can read per-version Forge/MC details when needed.
 */
export function useLimyrxClient() {
  const service = useService(LimyrxClientServiceKey)
  return useAsyncData(() => service.getManifest())
}
