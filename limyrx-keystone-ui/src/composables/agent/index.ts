...
export function useAgent(): AgentSession {
  ...
  async function send(userInput: string, options: Partial<RunAgentOptions> = {}) {
    ...
    if (!agentSettings.resolvedApiKey.value.trim()) {
    ...
    try {
      await runAgent(history, {
        apiKey: agentSettings.resolvedApiKey.value.trim(),
  ...
  // "Configured" requires all three fields (key, endpoint, model) to resolve
  // to a non-empty value. Any null/empty stored value falls back to the
  // shipped Limyrx Agent defaults, so the agent is available out of the box.
  const available = computed(() =>
    !!(agentSettings.resolvedApiKey.value || '').trim() &&
    !!(agentSettings.resolvedEndpoint.value || '').trim() &&
    !!(agentSettings.resolvedModel.value || '').trim())
...
export function installAgentDevLauncher(session: AgentSession) {
  if (!session.available.value) {
    console.info('[limyrx-agent] disabled (API key missing)')
    return
  }
  ;(window as any).__limyrx_agent = {
    send: (input: string) => session.send(input).catch((e) => console.error('[limyrx-agent]', e)),
    reset: () => session.reset(),
    abort: () => session.abort(),
  ...
  console.info('[limyrx-agent] ready — try window.__limyrx_agent.send("list my mods")')
}
