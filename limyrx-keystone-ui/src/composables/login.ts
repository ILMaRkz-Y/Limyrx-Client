...
export function useAllowThirdparty() {
  // Limyrx Client: always allow offline / third-party auth methods.
  // Upstream XMCL gates these behind "has a Microsoft account, developer
  // mode, or a non-strict OS locale", which hides the Offline / Ely.by /
  // custom Yggdrasil options on fresh installs without a Microsoft account.
  // That defeats the purpose of this launcher, so the gate is removed.
