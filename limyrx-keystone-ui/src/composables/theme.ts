import { updateM3Theme } from '@/vuetify'
...
import { useDark, useDebounceFn, usePreferredDark, useStyleTag } from '@vueuse/core'
...
export function useThemeWritter(
  ...
  /**
   * Use the OS desktop wallpaper as the launcher background. The path is
   * resolved by the main process (no CORS / sandbox concerns there) and then
   * imported through the regular media pipeline.
   */
  async function setBackgroundToDesktop() {
    const result = await windowController.getDesktopWallpaper()
    if (result.error || !result.path) {
      throw new Error(`Failed to read desktop wallpaper: ${result.error ?? 'unknown error'}`)
    }
    await setBackgroundImage(result.path)
  }
...
export function useTheme(
  ...
  watch(
    ...
    (newColor) => {
      ...
      // Regenerate full M3 tonal palette from the new seed color
      try { updateM3Theme(newColor) } catch { /* ignore */ }
    },
    { immediate: true },
  )
  watch(
