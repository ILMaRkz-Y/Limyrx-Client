<template>
  <!-- Splash screen — shows until loading sequence completes -->
  <SplashScreen v-if="showSplash" @done="showSplash = false" />
  <v-app v-if="!showSplash && !showSetup" class="h-full max-h-screen overflow-hidden" :class="{ 'dark': isDark }">
    <AppBackground />
    <!-- Dawn parallax background — only on home page -->
    <DawnBackground v-if="isHomePage" />
    <!-- Non-home: gradient overlay -->
    <div
      v-if="!isHomePage"
    ...
    >
      ...
      <main class="relative flex min-h-0 flex-1 flex-col overflow-auto" :class="mainClasses">
...
// UI Zoom — reactive from localStorage
const uiZoom = useLocalStorage('ui_zoom', 100, { writeDefaults: false })

// Apply UI zoom via Electron's native setZoomFactor (no CSS hacks)
function applyZoom(val: number) {
  try {
    ;(window as any).windowController?.setUiZoom(val)
  } catch { /* not in Electron */ }
}
applyZoom(uiZoom.value)
watch(uiZoom, (val) => applyZoom(val))
...
// Bind Ctrl/Cmd+Shift+A to toggle the agent chat panel. The agent ships
// pre-configured for every user, so the hotkey is always active.
useAgentChatHotkey()
