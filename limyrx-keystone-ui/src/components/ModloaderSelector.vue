  ...
  <div>
    ...
    <v-expand-transition>
      <div v-if="currentTab !== 'vanilla'">
        ...
        />
        <VersionInputLimyrx
          v-else-if="currentTab === 'limyrx'"
          data-testid="version-input-limyrx"
          :value="data.runtime.limyrx"
          @input="onSelectLimyrxVersion"
        />
      </div>
    </v-expand-transition>
...
import VersionInputLimyrx from '@/components/VersionInputLimyrx.vue'
...
const limyrx = useLimyrxClient()

const loaders = [
  { id: 'limyrx', name: 'Limyrx Client', icon: BuiltinImages.limyrx },
...
/**
 * A Limyrx Client version fully determines the runtime: the manifest pins the
 * Minecraft version and the exact Forge build the client runs on. Selecting
 * one therefore also fixes `runtime.minecraft` and `runtime.forge`, so the
 * regular install flow (`fix()`) installs exactly the right pair and the
 * content step only has to drop the mods / configs into the instance.
 */
function onSelectLimyrxVersion(version: string) {
  onSelectLimyrx(version)
  if (version) {
    const manifest = limyrx.data.value
    const entry = manifest?.versions[version]
    if (entry) {
      props.data.runtime.minecraft = entry.minecraft
      props.data.runtime.forge = entry.forge
    }
  }
}
</script>
