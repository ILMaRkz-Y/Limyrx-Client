...
import { useAgentChatBus, useAgentSetupDocUrl } from '@/composables/agentChat'
...
const setupDocUrl = useAgentSetupDocUrl()

function onAskAgent() {
  emit('close')
  if (!agentAvailable.value) {
    // Open the Limyrx setup guide when the built-in agent isn't ready.
