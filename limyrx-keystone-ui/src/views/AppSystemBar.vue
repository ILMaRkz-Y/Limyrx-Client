<template>
  <v-system-bar
    ...
    class="moveable static! flex w-full grow-0 gap-1 p-0 text-[.875rem]! dark:color-[#ffffffb3] pr-0"
    :class="isHomePage ? 'system-bar-glass' : 'bg-[transparent]!'"
    :style="{ 'backdrop-filter': `blur(${isHomePage ? 24 : blurAppBar}px) saturate(1.3)`, '-webkit-backdrop-filter': `blur(${isHomePage ? 24 : blurAppBar}px) saturate(1.3)` }"
  >
    <span
      v-if="back"
    ...
    <ServerStatusIcon host="play.hypixel.net" label="Hypixel" :port="25565" class="ml-1" />
...
import ServerStatusIcon from './ServerStatusIcon.vue'
...
/* Glassmorphism for system bar on home page */
.system-bar-glass {
  background: rgba(255, 255, 255, 0.08) !important;
  border-bottom: 1px solid rgba(255, 255, 255, 0.12);
}
:root.dark .system-bar-glass,
.dark .system-bar-glass {
