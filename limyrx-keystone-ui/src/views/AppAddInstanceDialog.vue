...
const limyrxService = useService(LimyrxClientServiceKey)
...
const onCreate = async () => {
  ...
  // Snapshot the Limyrx Client selection before `create()` resets the form.
  const limyrxVersion = creation.data.runtime.limyrx
  if (isBedrock) {
  ...
  // Limyrx Client content (mods & settings) — installed after the regular
  // Minecraft/Forge install so the game directory is ready to receive files.
  if (!isBedrock && limyrxVersion && newPath) {
    try {
      await limyrxService.installContent(newPath, limyrxVersion)
      notify({
        level: 'success',
        title: t('limyrxClient.installTitle'),
        body: t('limyrxClient.installBody', { version: limyrxVersion }),
      })
    } catch (e) {
      console.error('Failed to install Limyrx Client content:', e)
      notify({
        level: 'error',
        title: t('limyrxClient.installFailed'),
        body: `${e}`,
      })
    }
  }
