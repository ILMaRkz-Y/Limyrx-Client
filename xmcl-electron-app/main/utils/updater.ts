...
async function downloadAsarUpdate(
  ...
  const urls = gfw.inside
    ? [`https://github.com/Yassir2010-gif/Limyrx-Launcher/releases/download/v${version}/${file}`]
    : [`https://github.com/Yassir2010-gif/Limyrx-Launcher/releases/download/v${version}/${file}`]
...
async function hintUserDownload(): Promise<void> {
  shell.openExternal('https://limyrx.xyz')
}
...
async function downloadAppInstaller(
  ...
  const destination = join(app.getPath('downloads'), 'Limyrx Launcher.appinstaller')
  // Windows App Installer - update this URL if you host the .appinstaller file
  const url = 'https://github.com/Yassir2010-gif/Limyrx-Launcher/releases/latest/download/xmcl.appinstaller'
      ...
      Object.assign(executor, {
        ...
        createRequest: (options: any, callback: any) => {
          if (gfw.inside) {
            ...
            options.pathname = `/Soft_Mirrors/github-release/Yassir2010-gif/Limyrx-Launcher/LatestRelease/${basename(options.pathname)}`
...
export class ElectronUpdater implements LauncherAppUpdater {
  ...
  async #getUpdateFromSelfHost(): Promise<ReleaseInfo> {
    ...
    const response = await this.app
      .fetch(`https://api.github.com/repos/Yassir2010-gif/Limyrx-Launcher/releases/latest`, {
        headers: {
