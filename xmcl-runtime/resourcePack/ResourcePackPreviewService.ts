...
export class ResourcePackPreviewService extends AbstractService implements IResourcePackPreviewService {
  ...
  async loadModel(modelPath: string) {
    ...
    const model = await this.modelLoader.loadModel(modelPath).catch((e) => {
      this.warn(`Failed to load model ${modelPath}`, e)
      throw e
    })
    ...
    for (const [name, res] of Object.entries(this.modelLoader.textures)) {
      try {
        ...
        this.warn(`Failed to read texture ${name}`, e)
      }
    }
  ...
  async getBlockStates(gameVersion: string): Promise<BlockStateJson[]> {
    ...
    if (this.resourceManager.list.length === 0) {
      this.warn('No resource packs loaded for blockstate preview')
      return []
    }
    ...
    if (!vanilla) {
      this.warn('No vanilla jar loaded for blockstate preview')
      return []
    }
    ...
    try {
      ...
      this.warn(`Failed to read blockstates from ${gameVersion}.jar`, e)
      return []
    }
  }
}
