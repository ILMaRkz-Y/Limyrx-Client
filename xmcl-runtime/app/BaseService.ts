...
export class BaseService extends AbstractService implements IBaseService {
  ...
  async makeDesktopShortcut() {
    ...
    if (process.platform === 'win32') {
      const shortcutPath = join(desktopDir, 'Limyrx Client.lnk')
            ...
            if (/^\d+$/.test(userDir)) {
              ...
              if (await pathExists(configDir)) {
                ...
                const alreadyExists = existingList.some(
                  (s: any) => s && (s.AppName === 'Limyrx Client' || s.AppName === 'Limyrx' || s.AppName === 'X Minecraft Launcher' || s.AppName === 'XMCL')
                )

                if (!alreadyExists) {
                  const nextIdx = Object.keys(data.shortcuts).length.toString()
                  data.shortcuts[nextIdx] = {
                    AppName: 'Limyrx Client',
                    Exe: exePath,
                    StartDir: `"${startDir}"`,
                    icon: exePath,
                    ShortcutPath: '',
