...
export async function normalizeSkinData(url: string) {
  ...
  // Handle data: URLs (base64-encoded images from the skin upload dialog)
  if (url.startsWith('data:')) {
    const matches = url.match(/^data:image\/(png|jpeg|webp);base64,(.+)$/)
    if (!matches) {
  ...
  if (resolved.protocol === 'file:' || resolved.protocol === 'image:') {
    ...
    throw new Error('Unknown url protocol "' + resolved.protocol + '"! Require a file, https, or data URL.')
  }
}
