...
export class InstanceModsService extends AbstractInstanceDomainService implements IInstanceModsService {
  ...
  private async isProtectedFile(path: string, fileName: string): Promise<boolean> {
    const instanceService = await this.app.registry.getIfPresent(InstanceService)
    if (!instanceService) return false
    const instance = instanceService.state.all[path]
    if (instance?.systemFiles) {
      const relPath = relative(path, join(path, ResourceDomain.Mods, fileName)).replace(/\\/g, '/')
      return instance.systemFiles.some(sf => sf.path === relPath)
    }
    return false
  }
  ...
  async disable({ files: mods, path }: UpdateInstanceResourcesOptions) {
    ...
    for (const resource of mods) {
      ...
      if (resource.endsWith('.disabled')) {
        this.warn(`Skip to disable disabled mod file on ${resource}!`)
        continue
      }

      const fileName = resource.substring(instanceModsDir.length + 1)
      if (await this.isProtectedFile(path, fileName)) {
        this.warn(`Blocked disabling protected system file: ${fileName}`)
        continue
      }
