...
export class InstanceService extends StatefulService<InstanceState> implements IInstanceService {
  ...
  async duplicateInstance(path: string) {
    ...
    if (instance.locked) {
      this.error(new AnyError('InstanceLockedError', `Instance ${path} is locked and cannot be duplicated.`))
      return ''
    }
  ...
  async deleteInstance(path: string, deleteData = true) {
    ...
    const state = this.state.all[path]
    if (state?.locked) {
      this.error(new AnyError('InstanceLockedError', `Instance ${path} is locked and cannot be deleted.`))
      return
    }
  ...
  async editInstance(options: EditInstanceOptions & { instancePath: string }) {
    ...
    if (state.locked) {
      this.error(new AnyError('InstanceLockedError', `Instance ${instancePath} is locked and cannot be edited.`))
      return
    }
  ...
  async getInstanceSessions(path: string): Promise<GameSessionCollection> {
    const sessionsFile = join(path, SESSIONS_FILE)
    try {
