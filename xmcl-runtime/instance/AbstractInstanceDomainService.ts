  ...
  async uninstall({ files, path }: UpdateInstanceResourcesOptions) {
    ...
    if (instance?.systemFiles) {
      for (const sf of instance.systemFiles) {
        protectedRelativePaths.add(sf.path)
      }
    }

    await Promise.all(files.map(async (f) => {
      const dest = join(path, this.domain, basename(f))
      const relPath = relative(path, dest).replace(/\\/g, '/')

      if (protectedRelativePaths.has(relPath)) {
        this.warn(`Blocked deletion of protected system file: ${relPath}`)
        return
      }
