<template>
  ...
  >
    ...
    <span class="whitespace-nowrap text-center text-xs text-gray-500">
      {{ localized?.subtitle ?? '' }}
    </span>
    <v-progress-linear
      rounded
      color="blue"
      :model-value="total > 0 ? percentage : undefined"
      :indeterminate="total <= 0"
      height="6"
    />
    <span class="whitespace-nowrap text-center text-sm font-bold">
      {{ progress > 0 && total > 0 ? getExpectedSize(progress) + ' / ' + getExpectedSize(total) : '' }}
    </span>
  </div>
...
const localized = computed(() => task.value ? localizeTask(task.value) : null)
const { show: showTask } = useDialog('task')
const percentage = computed(() => total.value > 0 ? progress.value / total.value * 100 : 0)
