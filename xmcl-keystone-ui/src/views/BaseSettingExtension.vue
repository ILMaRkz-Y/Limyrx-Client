<template>
  ...
  >
    ...
    >
      ...
      <v-btn
        v-shared-tooltip="() => 'Files'"
        variant="text"
        :aria-pressed="targetQuery === 'files'"
        :class="{ 'v-btn--active': targetQuery === 'files' }"
        @click="navigate('files')"
      >
        <v-icon :start="targetQuery === 'files'" class="material-icons-outlined">description</v-icon>
        <span
          :style="{ width: targetQuery === 'files' ? '60px' : 0 }"
          class="overflow-hidden transition-all!"
        >
          Files
        </span>
      ...
      <v-btn
        v-shared-tooltip="() => 'Mods'"
        variant="text"
        :aria-pressed="targetQuery === 'mods'"
        :class="{ 'v-btn--active': targetQuery === 'mods' }"
        @click="navigate('mods')"
      >
        <v-icon :start="targetQuery === 'mods'" class="material-icons-outlined">extension</v-icon>
        <span
          :style="{ width: targetQuery === 'mods' ? '60px' : 0 }"
          class="overflow-hidden transition-all!"
        >
          Mods
        </span>
...
function navigate(target: '' | 'modpack' | 'appearance' | 'files' | 'mods') {
