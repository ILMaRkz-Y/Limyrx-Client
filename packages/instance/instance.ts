...
/**
 * A system file entry for tracking protected files in an instance.
 * These files are bundled with the launcher or pushed from admin server,
 * and users cannot delete them through the UI.
 */
export const SystemFileEntrySchema = z.object({
  /** Relative path from instance root (e.g. "mods/my-mod.jar") */
  path: z.string(),
  /** SHA1 hash of the file for integrity verification */
  sha1: z.string(),
  /** Source of this system file: 'bundled' | 'server-sync' */
  source: z.enum(['bundled', 'server-sync']).optional(),
})
...
export const InstanceDataSchema = z.object({
  ...
  /** Custom banner image URL for this instance */
  banner: z.string().catch('').default(''),
  ...
  /** When true, the instance is locked and cannot be edited, modded, or deleted through the launcher UI */
  locked: z.boolean().optional().catch(undefined),
  /** List of system files that users cannot delete. Protected by launcher. */
  systemFiles: z.array(SystemFileEntrySchema).optional().catch(undefined),
  /** Enable automatic server sync for this instance */
  serverSync: z.boolean().optional().catch(undefined),
  /** Polling interval in minutes for server sync (default 5) */
  serverSyncInterval: z.number().nonnegative().optional().catch(undefined),
