...
export async function computeInstanceEditChanges(
  ...
  // Handle banner URL transformation (same pattern as icon)
  if ('banner' in result && result.banner) {
    try {
      const bannerURL = new URL(result.banner)
      const path = bannerURL.searchParams.get('path')
      if (bannerURL.host === 'launcher' && bannerURL.pathname === '/media' && path) {
        result.banner = await getIconUrl(path)
      }
    } catch {
      // Invalid URL, keep as is
    }
