...
const ELECTRON_ENTRY = resolve(REPO_ROOT, 'limyrx-electron-app/dist/index.js')
...
const ELECTRON_BIN = process.platform === 'win32'
  ? resolve(REPO_ROOT, 'limyrx-electron-app/node_modules/electron/dist/electron.exe')
  : process.platform === 'darwin'
    ? resolve(REPO_ROOT, 'limyrx-electron-app/node_modules/electron/dist/Electron.app/Contents/MacOS/Electron')
    : resolve(REPO_ROOT, 'limyrx-electron-app/node_modules/electron/dist/electron')
...
export const test = base.extend<{
  ...
  launcher: async ({ launcherOptions }, use, testInfo) => {
    ...
    // Layout matches what xmcl-runtime/app/LauncherApp.ts expects for this
    // fork: LAUNCHER_NAME = 'limyrx' (xmcl-runtime/constant.ts), so the app
    // reads `{appDataPath}/limyrx/root` — which contains the game data path.
    ...
    if (!launcherOptions.bootstrap) {
      const launcherDir = join(appDataPath, 'limyrx')
      await mkdir(launcherDir, { recursive: true })
      await writeFile(join(launcherDir, 'root'), gameDataPath)
    }
    ...
    const app = await _electron.launch({
      ...
      args: [ELECTRON_ENTRY],
      cwd: resolve(REPO_ROOT, 'limyrx-electron-app/dist'),
      env: {
        ...process.env,
      ...
      timeout: 60_000,
      // NOTE: do NOT add `recordVideo` here. Playwright's electron integration
      // (>= 1.62) hangs this app's custom-protocol BrowserWindow navigation
      // when recordVideo is enabled: the window is created but loadURL never
      // starts (url stays "", isLoading() stays true forever). A/B-verified
      // on win32 08/2026 — with recordVideo the renderer never navigates,
      // without it the same launch boots in ~2s. The config-level
      // `use.video` (XMCL_E2E_VIDEO) still applies to any regular Page.
    })
